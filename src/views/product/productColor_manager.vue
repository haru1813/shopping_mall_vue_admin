...
export default {
    ...
    data: function () {
        return {
            ...
            harumarket_productColor_name: null,
            dataStores: dataStore(),
            productColor_manager_insertModal: null,
            productColor_manager_updateModal: null,
    ...
    async mounted() {
        this.$nextTick(() => {
            ...
            this.productColor_manager_insertModal = this.$refs.productColor_manager_insertModal;
            this.productColor_manager_updateModal = this.$refs.productColor_manager_updateModal;
            this.search.style.height = (screen.height / 5 + 0) + "px";
            this.result.style.height = (screen.height / 1.6 + 0) + "px";

            this.harumarket_productColor_name = this.$refs.harumarket_productColor_name;
        })
        ...
        this.myModalInstance = new bootstrap.Modal(this.productColor_manager_updateModal, {});
    },
    methods: {
        async searchF() {
            let req = {
                harumarket_productColor_name: this.harumarket_productColor_name ? this.harumarket_productColor_name.value : '',
            }

            let responData = await httpRequest("POST", "https://back1.haru.company/admin/productColor_manager_search", req, this.dataStores.authorization);
        ...
        insert() {
            var myModal = new bootstrap.Modal(this.productColor_manager_insertModal, {});
            myModal.show();
        },
        async insert_process() {
            const insertForm = new FormData(this.$refs.insertForm);
            let harumarket_productColor_name = insertForm.get('harumarket_productColor_name');

            if (harumarket_productColor_name == "") {
                toastr.error('상품 색상 이름을 입력하여 주십시오.');
                this.$refs.insertForm.querySelector('input[name="harumarket_productColor_name"]').focus();
                return;
            }
            ...
            let responData = await httpRequest("PUT", "https://back1.haru.company/admin/productColor_manager_insert", req, this.dataStores.authorization);
            if (responData.status != 200) {
                responData = await httpRequest("POST", "https://back1.haru.company/common/token_refresh", null, this.dataStores.authorization);
                if (responData.status == 200) {
                    this.dataStores.authorization = responData.data.token;
                    responData = await httpRequest("PUT", "https://back1.haru.company/admin/productColor_manager_insert", req, this.dataStores.authorization);
                }
                else {
                    toastr.error("로그아웃 되었습니다.");
                    this.dataStores.authorization = "";
        ...
        async update() {
            ...
            let req = {
                harumarket_productColor_index: Number(harumarket_productColor_index),
            }

            let responData = await httpRequest("POST", "https://back1.haru.company/admin/productColor_manager_view", req, this.dataStores.authorization);
            if (responData.status != 200) {
                responData = await httpRequest("POST", "https://back1.haru.company/common/token_refresh", null, this.dataStores.authorization);
                if (responData.status == 200) {
                    this.dataStores.authorization = responData.data.token;
                    responData = await httpRequest("POST", "https://back1.haru.company/admin/productColor_manager_view", req, this.dataStores.authorization);
                }
                else {
                    toastr.error("로그아웃 되었습니다.");
                    this.dataStores.authorization = "";
                ...
                let harumarket_productColor_name = this.$refs.updateForm.querySelector('input[name="harumarket_productColor_name"]');
                harumarket_productColor_name.value = res.harumarket_productColor_name;
        ...
        async update_process() {
            ...
            let harumarket_productColor_name = updateForm.get('harumarket_productColor_name');
            ...
            let req = {
                harumarket_productColor_index: harumarket_productColor_index,
                harumarket_productColor_name: harumarket_productColor_name,
            }

            let responData = await httpRequest("PATCH", "https://back1.haru.company/admin/productColor_manager_update", req, this.dataStores.authorization);
            if (responData.status != 200) {
                responData = await httpRequest("POST", "https://back1.haru.company/common/token_refresh", null, this.dataStores.authorization);
                if (responData.status == 200) {
                    this.dataStores.authorization = responData.data.token;
                    responData = await httpRequest("PATCH", "https://back1.haru.company/admin/productColor_manager_update", req, this.dataStores.authorization);
                }
                else {
                    toastr.error("로그아웃 되었습니다.");
                    this.dataStores.authorization = "";
...
<template>
    <div class="container-fluid h-100 w-100 p-0 m-0">
        <navComponent category="product" category_detail="productColor_manager" />
        <div class="row">
                                ...
                                <div class="row">
                                    <div class="col-3">
                                        <div class="input-group input-group-sm">
                                            ...
                                            <input ref="harumarket_productColor_name" type="text" class="form-control"
    ...
    <!-- 등록 모달창 -->
    <form ref="insertForm">
        <div class="modal" tabindex="-1" ref="productColor_manager_insertModal">
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="input-group input-group-sm">
                                        ...
                                        <input name="harumarket_productColor_name" type="text" class="form-control"
                                            placeholder="" aria-label="Username" aria-describedby="basic-addon1"
                                            style="height:31px;">
                                    </div>
                                </div>
    ...
    <!-- 수정 모달창 -->
    <form ref="updateForm">
        <div class="modal" tabindex="-1" ref="productColor_manager_updateModal">
            <div class="modal-dialog modal-dialog-scrollable">
                <div class="modal-content">
                    <div class="modal-header">
